...
### Session 2026-08-18 (iteration 8) — pricing component, row-click expand, list default view

- Q: What replaces the "PnL / Net" column label and its realized/net suffix? → A: Just **PnL**, signed and coloured with the row's currency symbol (`+ NT$ 168`). The realized-vs-net nuance moves to a tooltip on the detail panel; nothing states it inline.
- Q: How is an amount rendered anywhere in the app? → A: Through ONE shared `<Price>` component built on pure normalizer functions the ECharts formatters also call (constitution XIII). One precision policy, one symbol fallback, one red/green/grey palette, tabular figures, and a rounding-gated full-precision tooltip.
- Q: What does clicking an expandable row do? → A: Toggles it, from the same shared row-props helper as whole-row navigation. Child rows never collapse their parent; a row that both expands and has a detail page navigates.
- Q: What does the Portfolios list open with? → A: Name, PnL, Position — sorted last transaction descending, then state ascending. Position is computed server-side over every transfer, one query per page.

### Session 2026-08-25 (iteration 9) — chart polish, position badges, accumulated vs change columns

*Directives from the user's review of the shipped iteration 8, followed by the questions they raised. This session ran unattended: the questions marked **auto-answered** carry the recommended default and were verified against the real data where possible — override any of them and the affected FR is amended.*

- Q: (Directive) The Portfolios list Position column loses its header, and comma-separated holdings are hard to read → A: **Bug confirmed in HEAD** — the `holdings` column declares `autoWidth.header` but never a `title` (the same defect class as iteration 6's blank headers; fixed at the `PageTable` level so it cannot recur — a column's header defaults to its `autoWidth.header`). Holdings render as **badges**: one tag per asset, the quantity and the asset name in visibly different tones (FR-052).
- Q: (Directive) Chart tooltips must match the Balance Sheets list chart's tooltip → A: ONE shared tooltip builder — bold date title, a table with the series marker + name on the left and the right-aligned tabular value — used by the PnL tab, the Trend tab and the Balance Sheets charts, with every value produced by the Principle XIII normalizers (FR-053).
- Q: (Directive) PnL tab: remove the "PnL: … / Still holding: …" statements and the "Charted from the N transactions on this page…" note; Trend tab: remove the same note → A: Both tabs become chart-only (FR-054, FR-055). The realized-vs-net caveat survives only as an info icon in the tab bar; the detail page no longer lists holdings (FR-034 amended — the list's Position column and the Accumulated Position column carry them).
- Q: (Directive) Trend tooltip must use the pricing format → A: Values go through the shared normalizers (`− NT$ 1,234`) via the shared tooltip builder (FR-055).
- Q: (Directive) Transactions table: rename PnL → **Accumulated PnL**, Position → **Accumulated Position**; add **Tx PnL Change** and **Tx Position Change**; accumulated columns belong to transaction (parent) rows, change columns to transfer (child) rows → A: Six data columns in the order Time, Accumulated PnL, Accumulated Position, Tx PnL Change, Tx Position Change, Description. A parent row fills ONLY the two accumulated columns; a transfer row fills ONLY the two change columns (FR-056, supersedes FR-044).
- Q: What does the Trend tab's "Position" bar measure, given it "should have positive values — opposite to cost"? *(auto-answered)* → A: **The base-currency value moved into or out of positions: `position = −(cost + income)` per transaction.** Evidence: the shipped series sums raw *quantities* across whichever assets a transaction touches, which is meaningless across units — 119 of the 359 real transactions plot a negative grey bar beside a red cost bar (e.g. paying 1,579 PT-sUSDE for 1,256 DAI nets −323 "units"; spending 2,014 USDT as a cost plots −2,014). Under the money-valued definition every cost-only transaction plots a positive bar (the user's stated expectation), an income-only transaction plots the mirror (position sold down), and cost, income and position share ONE axis — the second quantity axis goes. In Waterfall mode the grey running total is the capital currently deployed (net invested). *(Supersedes the Session 2026-08-16c per-asset-series answer for the chart only; the TABLE keeps per-asset quantities.)*
- Q: The accumulated columns and both charts are computed over the loaded page; the note being removed was the only disclosure. Keep page scope? *(auto-answered)* → A: **No — accumulate over the WHOLE portfolio.** Once a column is labelled "Accumulated", a page-scoped total is wrong, not merely partial: 3 of the 55 real portfolios exceed the 25-row page (max 53 transactions) and their newest row would show the sum of the newest 25 only. The detail page fetches the portfolio's complete transaction set in one unpaginated request (the backend allows 500 per page; the largest portfolio has 53) and derives the running totals AND the chart series from it, independent of the table's page and sort (FR-057).
- Q: How is a holding badge composed, and where is it reused? *(auto-answered)* → A: One default AntD `Tag` per asset (Principle VI foreign-key display); inside it the quantity via `<Price>` in the strong text tone and the asset name in the secondary tone — the pricing component gains a muted-unit variant rather than being formatted around (Principle XIII). Badges wrap within the cell. ONE shared component serves the Portfolios list Position column and the Accumulated Position column.
- Q: What does a currency (cash) leg show in Tx Position Change? *(auto-answered)* → A: Nothing — Position means asset holdings (FR-034); a cash leg's movement is already stated by its Tx PnL Change. Unchanged from iteration 8.
- Noted and **explicitly out of scope** for this iteration (kept here so they are not lost): (1) deleting a saved view should close its tab immediately; (2) the pricing input's field order should become [amount | currency] instead of [currency | amount]; (3) the data-migration preview's Update tab does not make clear what exactly changed.
...
- **FR-042**: The **PnL tab** MUST be a line chart in the style of the Balance Sheets equity curve, whose final point equals the portfolio's realized PnL (closed) or net PnL to date (open) — guaranteed by FR-057 (the curve covers every transaction, not the loaded page).
- **FR-043**: The **Trend tab** MUST plot one x-axis point per transaction with three y-values — cost, income and position, all in the portfolio's base currency (position defined by FR-055). Negative values MUST extend **downward** (never absolute-valued), and a **"Waterfall" toggle** MUST switch between waterfall (cumulative) and plain-bar mode.
- **FR-044**: *(Superseded by FR-056 on 2026-08-25.)* The Transactions table column order was **Time, PnL, Position, Description**, with a **transaction** row showing accumulated balances and a **transfer** row its own change — the split survives; the columns are now named and separated per FR-056.
...
- **FR-034**: Holdings — per asset, the net sum of asset change amounts, omitting assets whose net is zero — MUST be computed server-side across all transfers and exposed by the API. *(Amended 2026-08-25: the detail page no longer lists them in the PnL tab; they reach the user through the Portfolios list Position column (FR-046) and the newest transaction row's Accumulated Position (FR-056/FR-057), which is the same figure by construction.)*
- **FR-035**: A stock split is recorded as an ordinary position-only transfer (`+N` units, blank PnL) with the ratio in the **transaction description** (FR-039 removed the transfer remark); no dedicated split entity or form helper is added. Holdings (FR-034) MUST therefore reflect splits automatically, and PnL MUST be unaffected by them.
...
- **FR-022**: The Transactions panel MUST render each transaction's transfers as **child rows in the same table, sharing the parent's columns**, following the inventory catalog pattern: a dedicated caret column toggling expansion, `indentSize={0}`, and `showExpandColumn: false` — NOT a nested table with its own header row. *(The original "collapsed row summarises its transfers" clause was superseded by FR-044 in iteration 7 and by FR-056 in iteration 9: a parent row shows accumulated balances, never a summary of its children.)*

- **FR-046**: The Portfolios list MUST carry a **Position** column showing the assets a portfolio still holds. Like the PnL aggregates (FR-031) it MUST be computed **server-side over ALL transfers**, and MUST cost **one query per page** rather than one per row — the list serializer exposes `holdings`, bulk-computed in `PortfolioViewSet.list()`. Cells render the holding badges of FR-052 *(amended 2026-08-25: no longer a comma-separated clamped string)*.
- **FR-047**: The Portfolios default ordering MUST be **last transaction descending, then state ascending**. State only breaks ties, but the tie is real: every never-transacted portfolio has a null timestamp, and those MUST list active-first rather than arbitrarily.
- **FR-048**: The Portfolios list default view MUST show **Name, PnL, Position**. Description, Base Currency, State, and the two transaction timestamps remain available through the Columns control and participate in saved views.
- **FR-049**: The PnL column MUST be labelled exactly **"PnL"** and rendered by the shared pricing component: explicit sign, red for a loss and green for a gain, the row's own currency symbol, format `+ NT$ 168`. The inline "realized"/"net" qualifiers MUST be removed everywhere — they repeated the State column on every row. On the portfolio detail panel the distinction survives as a tooltip.
- **FR-050**: Every monetary amount, price, and asset quantity in the application MUST be rendered by the shared `<Price>` component, built on pure normalizer functions that chart formatters call directly (constitution XIII). The per-page decimal trimmers, the rival red/green palettes, and the hand-copied chart symbol closures MUST be removed.
- **FR-051**: Clicking anywhere on an **expandable** table row MUST toggle its expanded state, via the same shared row-props helper that provides whole-row navigation, with identical interactive-element and text-selection guards. A child row MUST NOT collapse its parent; a row that both expands and has a detail page MUST navigate.
- **FR-052**: Holdings MUST render as **badges**: ONE shared component producing one default AntD `Tag` per asset, containing the quantity (through the shared pricing component, strong text tone, tabular figures, no sign — a holding is a balance) and the asset name in a visibly different, secondary tone; badges wrap within the cell. It is used by the Portfolios list **Position** column and the Transactions table **Accumulated Position** column. The Position column MUST render its header — `PageTable` MUST default a column's `title` to its `autoWidth.header` so a column that declares a header text can never render a blank one (extends FR-030 to every table).
- **FR-053**: Every chart tooltip in the finance domain MUST be produced by ONE shared tooltip builder in the same format as the Balance Sheets list chart: a bold date title, then a table with one row per series — the series marker and name on the left, the value right-aligned in tabular figures — with every value formatted by the Principle XIII normalizers (currency symbol, one precision policy, explicit sign for changes). The tooltip MUST stay pinned to the active x-axis value (not follow the cursor) and remain inside the chart container. The PnL tab, the Trend tab and the Balance Sheets charts MUST all call this builder.
- **FR-054**: The **PnL tab** MUST contain the chart only: no PnL figure line, no "Still holding" line, and no "Charted from the N transactions on this page…" note. The realized (closed) vs net-to-date (open, no market prices) caveat MUST remain reachable as an info icon in the card's tab bar while the PnL tab is active.
- **FR-055**: The **Trend tab** MUST contain the chart only (the page note is removed). Its **position** series MUST be the base-currency value moved into or out of positions, `position = −(cost + income)` per transaction — positive for a purchase, negative for a sale, exactly opposite to the transaction's cash flow — plotted grey on the SAME money axis as cost and income (the separate quantity axis is removed); in Waterfall mode its running total is the capital currently deployed. Tooltip values MUST use the pricing format via FR-053.
- **FR-056**: The Transactions table MUST carry, in this order, **Time, Accumulated PnL, Accumulated Position, Tx PnL Change, Tx Position Change, Description** (then Actions). A **transaction** row fills ONLY the two Accumulated columns (PnL as one signed figure with the base-currency symbol; Position as the FR-052 badges of the per-asset running totals) and leaves both Tx change columns empty. A **transfer** row fills ONLY the two Tx change columns (`pnl_change` signed with the currency symbol; `asset_change_amount` signed with the asset name, e.g. `+123 0050.TW`; a cash leg leaves Tx Position Change empty) and leaves both Accumulated columns empty. Every column MUST render its header (FR-030).
- **FR-057**: The Accumulated columns and BOTH charts MUST be computed over the portfolio's **complete** transaction set, never over the loaded page: the detail page fetches every transaction of the portfolio in one unpaginated request ordered by time (the backend page cap of 500 exceeds the largest real portfolio, 53, by an order of magnitude), derives the running totals and the chart series from that set, and maps the running totals onto the table's rows by transaction id — so the figures are independent of the table's page, page size and sort order, and the newest transaction's Accumulated PnL equals the portfolio's `net_value_change` from the API.
...
- **Transfer**: A single leg within a transaction. Key attributes: EXACTLY ONE of an asset leg (asset reference + signed asset change amount, ≥18 decimal places) or a currency leg (currency reference + signed currency amount); optional `pnl_change` in the portfolio's base currency (provided for cost/expense/income events; omitted for pure position changes); parent transaction reference. *(Remark removed in iteration 7.)*
...
- **SC-023**: Every column header on the Portfolios list is non-empty (asserted by a page test), and a `PageTable` column that declares `autoWidth.header` without a `title` still renders that header (asserted at the component level, so the defect class is closed for every table)
- **SC-024**: The Trend option's position series equals `−(cost + income)` for every transaction and the option has exactly ONE y-axis; against the real data, no transaction with a cost and no income plots a negative position bar (the shipped build plots 119)
- **SC-025**: For each of the 3 real portfolios with more than 25 transactions, the newest transaction row's Accumulated PnL equals the portfolio's `net_value_change` from the API, and the PnL curve's last point equals it too — verified against the live database
- **SC-026**: Both chart tooltips render the shared format — a bold date title and one row per series whose value carries the currency symbol and normalizer precision (e.g. `− NT$ 1,234`) — and the Balance Sheets chart tooltips are produced by the same builder; the "Charted from … on this page" locale key no longer exists in either locale
- **SC-027**: A transaction row renders values only in the two Accumulated columns and a transfer row only in the two Tx change columns; the Portfolios list Position cell and the Accumulated Position cell render one badge per held asset with the quantity and the asset name in different tones
