...
Entity Views from GitHub issue #19: save pre-configured filtering + sorting + column visibility/ordering + page size as named per-user, per-table **views**. New `EntityView` model + owner-scoped ViewSet in backend `core/` (`/api/v1/core/entity-views/`, incl. bulk `reorder/`); frontend `components/EntityViews/` (serialization, `useEntityViews`, `ViewTabs` `[+] tabs… [View]` row via a new PageTable `viewBar` slot, View dropdown with Save/Duplicate/Edit, staged ManageViewsModal). URL is source of truth for the active view: `view[<tableKey>]=type=inline&…` or `type=saved&id=…&overrides` (contracts/view-url-serialization.md); session tabs in sessionStorage; virtual undeletable "Tabular" default tab; dirty = normalized config vs baseline. Adopted on all 5 entity list pages (finance currencies/accounts/exchange-rates, inventory catalog/scenarios). See [spec.md](specs/016-entity-views/spec.md), [research.md](specs/016-entity-views/research.md), [data-model.md](specs/016-entity-views/data-model.md).
