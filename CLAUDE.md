...
Inventory app enhancements (GitHub issue #39), three independent changes: (1) **accumulated-cost ownership** — new `CostFactor.user_managed` boolean (migration 0020): any manual edit to an accumulated line (incl. clear→0) freezes it as user-managed — stored verbatim on create (the server derives ONLY when the payload carries no accumulated factor; the old create-time "system-managed" rejection is dropped), never auto-recalculated on any later edit; the form's reconcile effect now re-derives AUTO rows live (fixing their stale-value latency) but never touches user-managed ones, keeps user-managed rows whose currency lost its priced items, and the per-line Reset restores the derived Σ + auto state; (2) **length unit default cm** — `DEFAULT_FAMILY_UNIT` map + `defaultUnitFor(family)` in the inventory service, used at ParameterRowsEditor's three default-unit sites (dropdown order and canonical mm storage untouched); (3) **catalog default pins** — `acquisition_summary` seeded `pin: 'left'` beside `__caret`, Actions stays right, column key bumped to `inventory-catalog-v8`. See [plan.md](specs/018-inventory-enhancements/plan.md), [spec.md](specs/018-inventory-enhancements/spec.md), [research.md](specs/018-inventory-enhancements/research.md), [contracts/acquisitions-api-delta.md](specs/018-inventory-enhancements/contracts/acquisitions-api-delta.md).
