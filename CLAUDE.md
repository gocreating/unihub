...
Inventory domain from GitHub issue #33: an entity-centric Django app (`inventory`) + frontend section for cataloging stockable/consumable **Items**, recording how each was obtained (**Acquisition**), and per-situation planning via **Scenario** (preparation checklists), **Constraint** (mutual-exclusive / required / weight-limit, evaluated server-side), item-in-item **containment** (per-scenario, cycle-checked), and position review. Shipped iterations 1–2 (commit `2fc0106`: acquisition-first creation, per-field currency via finance API, unit normalization). **Iteration 3 (2026-07-11)** in progress: cost→Acquisition (cost/discount/tax_refund/net_cost; item.cost removed), deprecate_time + derived status (Deprecate/Restore; archived_at/status removed), sku_price/total_price, +volume (mL/L), quantity required/1, −method/−model/−serial, source auto-complete, item **card view** + ≥1-item + default card, standalone acquisition edit page, and Constitution **v1.14.0** page/modal button rules (no Cancel on standalone pages; modal Cancel-left/dirty-guard/RWD) + blank-header/placeholder bug fixes. Migrations `0005_iter3_fields` (+backfills) / `0006_reseed_system_attrs`. Follows the Finance reference: concrete models + seeded system AttributeDefinitions, `ModelViewSet`s on `core/` filter/order/pagination, PageTable + EntityToolbar frontend. See [plan.md](specs/014-inventory-app/plan.md) and [research.md](specs/014-inventory-app/research.md).
<!-- SPECKIT END -->
