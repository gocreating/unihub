...
- **FR-003**: System MUST present a **single merged "Catalog" page** (Items and Acquisitions lists combined) as an **expandable tree table** on `PageTable`: each **acquisition is a parent row** (populating acquisition columns) that expands to reveal its **item child rows** (populating item columns). Expansion MUST use a **caret icon** (`CaretRight`/`CaretDown`), and the tree MUST be **expanded by default** (all items visible on load). Columns are the **union** of acquisition columns and item columns, with **`Source` and `Name` as two separate columns** (acquisition rows populate `Source`; item rows populate `Name`) — acquisition columns: `request_time` ("Requested"), `obtained_at`, net_cost, item count; item columns: spec, size, weight, length, width, height, `quantity`, `sku_price` (+currency), `url` (clickable), status, `deprecate_time`; a parent row leaves item columns empty and vice-versa. There is **no "Acquisition" badge** on parent rows. **Column widths MUST dynamically fit their content** (measured per column across rows) — including the **Actions column** (buttons never clip or overflow); no clipped or over-narrow cells. **Every column MUST be filterable and sortable via the toolbar.** Because the view is a tree, filtering/sorting is **level-aware**: acquisition columns filter/sort the parent tree (default order **acquisitions ↓ `obtained_at`**); applying a filter or sort on an **item-level column flattens** the Catalog to a **flat, ungrouped item list** for that operation, and clearing item filters/sorts restores the tree. **Every column MUST render its localized header** (no blank headers), and absent values MUST use the single standard placeholder "—" (Principle VI). The table MUST set **`columnEmptyText={false}`** so ProTable's built-in "-" is disabled and the em-dash is the sole placeholder. **Item rows have no "Edit" action** (items are edited via the acquisition edit page); item rows keep Deprecate/Restore and Delete; acquisition rows keep Edit and Delete.
...
- **FR-022**: The Add-Item modal (and any create/edit modal) MUST: place the **Cancel button left-most**; **not close on outside click while the form is dirty**; and **stack its fields to a single full-width column on narrow screens** (responsive to the actual content/modal width, not only the viewport). When editing an item, the modal is pre-filled with the item's current values.
