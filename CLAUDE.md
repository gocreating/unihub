...
Inventory domain from GitHub issue #33: an entity-centric Django app (`inventory`) + frontend section for cataloging **Items**, recording how each was obtained (**Acquisition**, with 1..N signed **CostFactor**s → per-currency `net_cost`), and per-situation planning via **Scenario**/**Constraint**/containment. Iterations 1–12 shipped (acquisition-first creation, cost factors, merged "Catalog" tree page with server-side filter/sort/pagination + flatten-on-item-filter, data_io registration, HTML legacy import, width/sort fixes — see spec.md Clarifications for the full trail). **Iteration 13 (2026-07-12)** in progress — **Catalog derived columns & density** (constitution v1.18.0 two-row datetime): three read-only derived columns — **"Item"** (name-link primary / spec secondary), **"Parameters"** (Tag badges: color, weight, length, width, height, volume, size), **"Acquisition"** (`{source} {net cost}` primary / `request ~ obtained` range secondary; flat mode shows it on item rows via new nested `net_cost` on `AcquisitionSummarySerializer` → OpenAPI/type regen); default visible = **Acquisition, Item, Quantity, SKU price, Parameters** (all real columns hidden-by-default but toggleable — fixes the missing-URL-column dropdown bug; column key bumped to v3); Requested/Obtained/deprecate_time render **two-row datetime**; **item rows lose Delete** (Deprecate/Restore only). No migrations. See [plan.md](specs/014-inventory-app/plan.md) and [research.md](specs/014-inventory-app/research.md).
<!-- SPECKIT END -->
