...
- **FR-003**: System MUST present a **single merged "Catalog" page** (Items and Acquisitions lists combined) as an **expandable tree table** on `PageTable`: each **acquisition is a parent row** (populating acquisition columns) that expands to reveal its **item child rows** (populating item columns). Expansion MUST use an **arrow (▸/▾) icon**, not a plus/minus. Columns are the **union** of acquisition columns and item columns, with **`Source` and `Name` as two separate columns** (acquisition rows populate `Source`; item rows populate `Name`) — plus request/obtained dates, net_cost (acquisition) and spec, size, weight, length, width, height, status, `deprecate_time` (item); a parent row leaves item columns empty and vice-versa. There is **no "Acquisition" badge** on parent rows. The **Actions column MUST be sized to fit its content** (buttons never clip or overflow). The view supports search, sort, filtering, column configuration, and pagination; the default order is **acquisitions sorted descending by `obtained_at`**. **Every column MUST render its localized header** (no blank headers), and absent values MUST use the single standard placeholder "—" (Principle VI) — no mixed placeholder styles. The table MUST set **`columnEmptyText={false}`** so ProTable's built-in "-" is disabled and the em-dash is the sole placeholder. **Item rows have no "Edit" action** (items are edited via the acquisition edit page); item rows keep Deprecate/Restore and Delete; acquisition rows keep Edit and Delete.
...
- **FR-006c**: The acquisition form's payment panel MUST be titled **"Cost"** (not "Cost Factors") and placed at the **end of the form, below the Items panel**. Its header carries the **Add** action. Each factor row shows, left-to-right: a **drag handle** (manual factors only) · the `type` (free-text/suggestion combobox) · the **`value` + `currency` composed as a single control with the `value` right-aligned** · a trailing action (**reset** on accumulated rows, **remove** on manual rows). Rows MUST **fill the container width**; when stacked on a narrow content area they MUST have a **vertical gap** between rows. The derived per-currency **`net_cost` MUST render in the panel footer, labelled "Total"** (one line per currency).
...
- **FR-021**: Standalone detail/create/edit pages (Scenario detail, Acquisition create, Acquisition edit) MUST use a **breadcrumb** for navigation rather than a back button, and MUST NOT render a Cancel button (the user navigates away via the breadcrumb). The acquisition create AND edit pages MUST title their form section **"Acquisition"** (not "New Acquisition"). Breadcrumbs: create = **Acquisitions / New Acquisition**; edit = **Acquisitions / {acquisition.id} / Edit Acquisition** (three crumbs, the middle being the acquisition id).
