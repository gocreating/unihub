...
Finance portfolio management from GitHub issue #14: add Asset, Portfolio, Transaction, and Transfer entities with full CRUD. Portfolios have per-portfolio base currency (immutable, referencing the finance Currency model), lifecycle state (active/closed), and auto-derived first/last transaction timestamps. Transfers are inline expandable rows in the Transactions table; each transfer records a signed asset change amount plus an optional Value Change in the portfolio's base currency. Iteration 2 (shipped): Portfolios list Name + View are real hyperlinks (row Edit/Delete removed); portfolio detail has a breadcrumb and a "Portfolio" panel with `PanelHeaderActions`. **Iteration 3 (2026-08-13)** in progress — three streams per Clarifications Session 2026-08-13: (1) model amendments — drop `Asset.category`; add `Portfolio.description`, `Transaction.chain_id`/`tx_hash`, `Transfer.remark`; widen Transfer decimals to (38,18) for wei-level legacy amounts (migration 0012); (2) legacy CSV import — idempotent, strictly additive `import_legacy_finance` management command porting 38 assets / 55 portfolios / 359 transactions / 837 transfers from the untracked `migration/` CSVs (NEVER commit them — real personal financial data), legacy references reused as PKs, amounts ÷ 10^decimals, `UPDATE_POSITION` → blank Value Change, portfolio state from the "[Active]" name prefix (names verbatim); (3) policy compliance — fix the transactions 500 (the three 013 viewsets' `filterable_fields` use the pre-016 contract: `lookup` must be the ORM field path, not an operator), adopt entity views + quick search on Assets/Portfolios pages, quick search on the portfolio-detail Transactions panel, replace all `Modal.confirm` with the shared `confirmDialog`. See [plan.md](specs/013-finance-portfolio-management/plan.md), [research.md](specs/013-finance-portfolio-management/research.md) (I3-1…I3-7), [data-model.md](specs/013-finance-portfolio-management/data-model.md), [contracts/api.md](specs/013-finance-portfolio-management/contracts/api.md), [quickstart.md](specs/013-finance-portfolio-management/quickstart.md) (import runbook).
<!-- SPECKIT END -->
