...
5. **Given** an acquisition, **When** the user adds or removes an item, **Then** the acquisition's item cards update accordingly, and the derived value of the `accumulated` cost factor (if not manually overridden) reflects the new item-total sum.
...
- **FR-006b**: An acquisition's payment is modelled as **1..N cost factors**. Each cost factor has a **signed `value`** (negative reduces), a `currency` (code string), and a `type` label ∈ {accumulated, shipping, discount, tax_refund, paid_by_other, other} that is **informational only and does NOT affect the sign**. The derived read-only **`net_cost` is grouped by currency**: for each currency, the sum of that currency's factor values (no FX conversion). An acquisition MUST have **≥1 cost factor**; on create, one factor of type **`accumulated`** is auto-inserted with `value` defaulting to the **sum of the items' `total_price`** (Σ `sku_price × quantity`) — the user may **override** the value or **reset** it to the derived sum.
- **FR-007**: System MUST let the user view, from an acquisition, its items and its **cost factors** with the derived per-currency `net_cost`, and MUST allow adding/editing/removing items and cost factors on an existing acquisition, which MUST be **editable on its own standalone page**.
...
- **CostFactor**: A signed line of an acquisition's payment. Attributes: `value` (signed decimal — negative reduces net cost), `currency` (code string, finance-API picker), `type` (informational label ∈ {accumulated, shipping, discount, tax_refund, paid_by_other, other}). The **value carries the sign**; the type does not. Belongs to exactly one Acquisition (composition). The `accumulated` factor's value defaults to the sum of the acquisition's item `total_price`s and is user-overridable/resettable.
