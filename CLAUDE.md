...
Multiple sticky columns (GitHub issue #37): generalize column pinning from the two view-wide "pin first/last visible column" booleans to **per-column pin state** (`ColumnDef.pin?: 'left' | 'right'`) so any number of columns can be pinned to either table edge. Frontend-only — shared `EntityToolbar` infra (`useColumnConfig`: pin-group-major display ordering, `fixedForKey()`, `pinFingerprint` remount key; `ColumnPanel`: per-row pin-left/pin-right pushpins, apply-gated) + mechanical updates to the 6 consumer pages (finance accounts/currencies/exchange-rates/balance-sheets, inventory catalog/scenarios; catalog defaults move from `defaultSticky` to `pin` seeds on `__caret`/`actions`). No persistence (column settings stay per-visit, verified — the `_key` params are unused). Constitution Principle XII amendment → v1.23.0 (remount-key + label-patch bullets reference the per-column model). See [spec.md](specs/017-multiple-sticky-columns/spec.md), [research.md](specs/017-multiple-sticky-columns/research.md), [data-model.md](specs/017-multiple-sticky-columns/data-model.md).
