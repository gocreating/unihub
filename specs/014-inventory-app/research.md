...
## Iteration 43 research (drag anchor, nest highlight, search marks, 2026-07-19)

- **R43.1 — Jump (probed)**: a headless 1px-activation probe shows correct tracking (overlay == row + delta), so the user's jump comes from environment-dependent ancestor transforms (ProLayout sider/menu animations, Splitter) breaking the overlay's fixed positioning — the documented dnd-kit remedy is portaling the overlay to document.body; harmless where the bug didn't manifest, curative where it did.
- **R43.2 — Nest visual**: the existing projection already yields `container_id` per move; rendering state extends from `{gapIndex, depth}` to include the prospective container + its visible-descendant set, letting OrgRow tint (AntD primary tokens at low alpha) without new geometry work.
- **R43.3 — Highlight scope**: server search covers name/alias/spec; the context line renders source+dates (dates rarely match but highlighting is harmless and consistent).

## Iteration 44 research (anchored list prices, block-total leak, RM→MYR, 2026-07-19)

- **R44.1 — Root cause A (MEASURED, live parser run)**: five of the six wrong-SKU reports share the **block-total leak** — the header row's paid cell is `own` in its own row (rowspan spans DOWNWARD), so `build_from_rows` fed it to `own_prices` AND to the accumulated override; the header ITEM inherited the whole block total as its sku (3126/2260/437/1848/1273, plus unreported 漁夫帽 2284 and 床墊 19034). The (c')-pass comment even documented the leak, relying on later passes to override it — which these blocks never trigger. Precise kill signal: a subsequent row in the SAME acquisition that carries (non-own) price text AND becomes an ITEM proves the header cell was a multi-item total; factor/continuation rows under the span do NOT kill it (single-item acquisitions keep sku = paid).
- **R44.2 — Root cause B (survey, 193 hits)**: per-row 備註 price key-values are colonless in the older sheets (`原價 680`, `單價 449`). Iteration 35 required the colon to stop PROSE extraction (雨傘王 `大傘，可兩人撐，原價850，搭配活動折價125`) — but the segment split (iteration 40) provides a sharper discriminator: an ANCHORED segment (the segment is nothing but the price expression) is a key-value; prose keeps extra words in its segment. Survey confirms every anchored 原價 hit is a genuine per-row list price; 雨傘王 stays safe because its row's own paid outranks the new 原價 tier (hierarchy, not regex, protects it).
- **R44.3 — Camping Flying (MEASURED)**: the shared listing uses descriptive fragments + color annotations (`大箱子(黑色)`); parens broke `RE_NAME_LIST_PRICE` entirely (no entries). Annotation tokens resolve targets (黑色 ∈ the 50L item name); elimination resolves the rest (箱子 → the remaining 21L). The remark cell is rowspan-carried → the blob held DUPLICATE lines; without dedupe the second `大箱子(黑色)原價 1380` copy would fall through shortening (箱子) onto the wrong item — dedupe is mandatory, and it's also why the stale DB had 21L = 1380.
- **R44.4 — Factor-row carried price (MEASURED)**: 2021:31's 運費及折扣 row read the CARRIED 3126 as its factor value → bogus `discount 3126 TWD` in the DB (net unaffected only because accumulated overrides). Value must come from OWN cells; the existing fully-consumed-leftover check doubles as the guard for remark-derived values (multi-number arithmetic remarks → value None, text preserved).
- **R44.5 — Quantity units (survey)**: `* N <unit>` occurs with 件/組/個/顆/條/包/盒/雙 (noise: 折/吋/=/c excluded — 折 is discounts, 吋 is a dimension). The 件-only classes silently dropped qty for 顆/個/組/條 rows (`單價499 * 3顆`, `單價 39 * 2 個`, `原價79 * 2條`).
- **R44.6 — RM→MYR**: finance now holds MYR (馬來西亞令吉, symbol RM — user-seeded this iteration). The parser's `[A-Za-z]+` currency group already captured `RM` but nothing normalized it → literal `"RM"` currency code in the DB. Importer-side aliasing (like RMB→CNY) keeps the parser standalone and finance authoritative. `RMB` must stay checked before `RM` (substring order already guaranteed by dict insertion order in `parse_currency`; the anchored group captures the full token so no ambiguity).
- **R44.7 — Composite key (survey)**: `color & size:` appears exactly once across all sheets; other `&` lines are names/arithmetic (`L & XS 各 499…`) that must NOT match — hence the keyword-anchored composite regex and the trailing-size-token vocabulary split (`65 土黃L` → `65 土黃` + `L`).

## Iteration 45 research (item-surface polish + tab titles, 2026-07-19)

- **R45.1 — Emoji offset (to probe)**: KeyEmoji already carries the iteration-41 treatment (inline-flex, `vertical-align: middle`, line-height 1), yet the user still sees a vertical offset in item rendering. `vertical-align: middle` positions the span relative to the parent baseline + half x-height — inside the Tag's OverflowTooltip (an ellipsized inline-block context with 11px text) that computation visibly misses. Plan: measure emoji vs label glyph boxes on the live app, then stop relying on vertical-align entirely — the pair renders inside an `inline-flex; align-items:center` row, which centers boxes irrespective of font metrics.
- **R45.2 — Name link**: the scenario list's Name cell is `<a onClick={navigate}>` with no `href` — RIGHT-click/middle-click/Cmd+click do nothing useful. `<Link to>` renders a real href AND keeps SPA navigation; precedent: iteration-27 side-nav links (FR-034), catalog Edit href (iteration 19).
- **R45.3 — Modal parameters**: organize-pane rows already render parameter pairs (RowContent → ItemDisplay `showParameters` + `parameters`); the Add modal's ItemDisplay call simply omits them. The modal's rows come from the same items list endpoint the catalog uses, whose serializer includes `parameters` (emoji + range fields) — pass-through, no API change.
- **R45.4 — Caret**: the tree row is `align-items: flex-start` with `marginTop: 4` nudges on the caret and holder — a fixed-pixel approximation that misses the primary line's true center (name line-height ≈ 22px). First-line centering via a fixed-height flex box matches the iteration-38 icon precedent and stays correct for multi-line rows (caret should not float to the middle of a 3-line block — it anchors the FIRST line, like AntD Tree switchers).
- **R45.5 — Titles**: no runtime title management exists (static `<title>Unihub</title>`). A `usePageTitle` hook (set + restore in `useEffect`) is the smallest reusable primitive; scenario list uses the localized menu label, detail the scenario name (list label while loading). Other pages stay as-is per the user's scope.

## Iteration 47 research (deprecate-modal preview, 2026-07-19)

- **R47.1**: the Deprecate modal shows only a generic confirm sentence — no item identity — while `deprecateTarget` already holds the FULL Item record (name/alias/url/spec/remark/parameters from the list payload). Rendering the shared ItemDisplay (with ParameterTag pairs) is a pass-through; no fetch, no API change, no new i18n chrome.
