...
- **FR-046**: The Portfolios list MUST carry a **Position** column showing the assets a portfolio still holds. Like the PnL aggregates (FR-031) it MUST be computed **server-side over ALL transfers**, and MUST cost **one query per page** rather than one per row — the list serializer exposes `holdings`, bulk-computed in `PortfolioViewSet.list()`. Cells clamp to two lines per Principle VI.
