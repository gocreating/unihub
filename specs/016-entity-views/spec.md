...
- Q: Is the default view special or a plain view? → A: A plain view — modifiable, renamable, and savable like any saved view; only deletion is excluded (it is the guaranteed fallback). Each page provides its initial name: "Table" generically, "YTD" for the inventory catalog. *(Superseded 2026-08-12: no page names its default view — every entity type's default view is "Table" until the user renames it.)*
...
- Q: Must the view tab row always render? → A: No — when a table has only its default view and no other tabs or URL view state, the row hides by default; a compact toolbar affordance (which also carries the unsaved-changes indicator) reveals it on demand, and the row auto-shows whenever a second view/tab exists. *(Superseded 2026-08-12: the row is ALWAYS shown — the auto-hide and its affordance are withdrawn entirely.)*
...
- Q: Should the catalog keep shipping its own default view (the year-to-date filter, the obtained-descending sort, 50/page, the name "YTD")? → A: No — remove all of it. The page contributes its COLUMNS; the stored default view supplies filters, sorting and page size. An account with no stored default sees an unfiltered, unsorted catalog at the standard page size.
- Q: A deep link to another view left the default tab showing the unsaved indicator. What is the rule? → A: A tab's configuration is its own view's, whether or not that tab is the one on screen. The default tab holds its stored view's configuration from the moment the view list resolves; only the tab the user is actually looking at follows the table.
- Q: "There shouldn't be a flash for views" — does the table's data wait too? → A: No, only the tab row. The row paints once, complete; the table keeps loading immediately as it does today.

### Session 2026-08-12c

- Q: "Apply this view pattern to all entity views" — what is the pattern, and which page defines it? → A: The catalog is the reference. A page contributes its COLUMNS only: no seeded filter, no seeded sorting, the shared default page size, and no page-provided default-view name. Whatever a table should open with belongs in the account's stored default view.
- Q: The other four pages already seeded nothing — is there anything to do? → A: Yes. Each hand-copied the same baseline object, page size included, and any drift between that copy and what the table actually starts with reads as unsaved changes on an untouched view. The baseline becomes a shared helper, and each page's arrival behaviour is locked by its own test.

### Session 2026-08-12d

- Q: Should the view row still hide itself when a table has only its default view? → A: No. The row is ALWAYS shown, on every page. The auto-hide, its reveal affordance and the persisted "revealed" preference are withdrawn entirely (FR-025 replaced).
- Q: Which name does a default view carry, now that no page seeds a configuration? → A: "Table" (localized) for every entity type. No page may supply a name — the option is removed, not merely unused — and the user renames it like any view. Views already stored under another name keep it.
...
1. **Given** a user visits an entity table with no saved views and no view state in the URL, **When** the page loads, **Then** the view row is shown with the table's default view — named "Table" until the user renames it — as a pinned, active tab.
...
- **FR-009**: Every entity table with the standard operations toolbar MUST provide a view-control row above that toolbar, laid out left to right as: the open view tabs, then a kebab menu fixed at the row's right edge. (The kebab replaces both the round-2 "+" button and the round-2 "View" control.)
...
- **FR-021**: A view configuration that references columns or attributes no longer available MUST degrade gracefully: unavailable parts are ignored and the remainder of the configuration applies. A column the configuration does not mention — one the page discovered after that configuration was written — MUST take the position the page declares for it, relative to the columns the configuration does mention, NOT be appended after them. Every comparison of two configurations MUST place such columns the same way, or two descriptions of the same table differ purely by when each was captured (the cause of FR-036's failure and of a permanent false indicator).
...
- **FR-039**: Every entity table MUST follow one view pattern, with no per-page variation: the page supplies its columns, and the account's stored default view supplies filters, sorting and page size. A page MUST NOT ship a default view of its own, and MUST NOT restate the default page size — the baseline it hands to the view tabs and the state the table actually opens with come from ONE definition, because any difference between them is reported as unsaved changes on a view nobody touched.
- **FR-038**: The view row MUST appear complete or not at all. Until every tab the visit will show is known, the row MUST reserve its space and show no tabs; tabs MUST NOT appear one after another as saved views arrive. The table's own data is not covered by this requirement — it loads as soon as it can.
...
- **FR-025**: The view row MUST be shown on every entity table, always — regardless of how many views exist, how many tabs are open, or what the URL addresses. There is no hidden state, no reveal affordance and no display preference to remember. *(This requirement replaces the round-2 auto-hide, withdrawn 2026-08-12.)*
...
- **SC-021**: All five entity tables behave identically on arrival: each issues its first request with no filter, no ordering and the shared default page size; each applies its stored default view when one exists; and none shows an unsaved indicator on arrival.
- **SC-020**: Opening a table shows its tab row once, complete: the number of tabs never changes after the row first appears, and a deep link to any view leaves every other open tab without an indicator.
...
- Tabs are per-visit, not per-session: nothing about the open-tab list is persisted, so a refresh or a navigation rebuilds the row from the account's pinned views plus the URL (FR-018). Nothing about the view row persists across a reload — round 13 withdrew the last such preference with the auto-hide.
